...
function priorityScore(lead: Lead): number {
  ...
  if (getCallScript(lead.id)) score += 5;
  return score;
}
...
export default function CallListPage() {
  ...
  const openLead = openLeadId
    ...
    : null;
  const openScript = openLeadId ? getCallScript(openLeadId) : null;
    ...
    <>
      ...
      <div className="flex items-center gap-3 pb-4">
        ...
        <span className="ml-auto font-mono text-xs text-muted-foreground">
          {callList.filter((l) => getCallScript(l.id)).length} scripts ready
        </span>
      </div>
        ...
        <Table>
          ...
          <TableBody>
            {callList.map((lead) => {
              ...
              const t = tier(score);
              const script = getCallScript(lead.id);
              return (
                <TableRow
                  key={lead.id}
